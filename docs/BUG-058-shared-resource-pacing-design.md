...
**What `PACING_GAP_MS = 6_000` is and isn't.** (This paragraph originally said `3_000` — a leftover from an earlier draft; the value that actually shipped is `6_000`, the 10-RPM figure derived just above. Corrected 2026-08-14 per the M27 Phase 0 constants audit, which flagged the inconsistency.) It shipped as a single conservative constant for v1, not tuned per-provider — free-tier RPM figures researched this session range from Groq's 30 RPM (≈2s/request sustainable) to OpenRouter's free 20 RPM (≈3s) to Gemini Flash's typically lower free-tier RPM (varies by model). A single conservative value was a deliberately simple starting point that de-clusters bursty concurrent use without being tuned to any one provider's exact window — not claimed to be optimal, and cheap to revisit once real post-ship data exists. **M27 H2 did that revisit:** `model-pacing.ts` now keys the gap on the model's provider, giving Groq a 2s gap and OpenRouter a 3s gap (their own documented rates) while every other provider keeps the conservative 6s default — one-directional, so no provider is ever paced *less* than its documented rate allows.
...
**Shipped.** One correction found during implementation, one field found during real research, everything else built as designed:

- **The Gemini field name was slightly off in the original research.** Real Gemini 429 bodies (confirmed via a fresh web search before writing the parser, not assumed from memory) put "PerDay" in `google.rpc.QuotaFailure.violations[].quotaId` (e.g. `"GenerateRequestsPerDayPerProjectPerModel-FreeTier"`), not `quotaMetric` as this doc originally said — `quotaMetric` is a bare resource path with no window information in it at all. `isGeminiDailyQuota()` (`providers/gemini.ts`) checks `quotaId` first, `quotaMetric` too as a defensive fallback in case a future response shape moves the text.
- **`toProviderError` in `openai-compatible.ts` gained a `providerId: AIProviderId` parameter** (it only had `displayName` before) — `resetsAtFrom()` needs to know which of the six OpenAI-compatible providers this is, since only Groq/OpenRouter have a real-or-documented reset signal. All three call sites plus the existing `providerErrorDetail.test.ts` calls were updated to pass it.
- **`markPeriodExhausted` gained an optional 5th `resetsAt?` parameter** (`model-cooldown.ts`) — an explicit `retryAfterMs` still wins outright (a direct instruction beats an estimate, same rule as `markRateLimited`), `resetsAt` only fills in when no `retryAfterMs` exists, and the existing `PERIOD_EXHAUSTED_MAX_MS` 24h cap still backstops it either way.
- **A test-construction correction, caught immediately, not shipped wrong**: the first version of the resetsAt provider tests constructed rate-limit failures with a bare `new APIError(429, ...)`, which — for `openai-compatible.ts` specifically — takes the WRONG branch inside `toProviderError`: a real 429 always arrives as the SDK's own `RateLimitError` subclass, checked before the generic `APIError` catch-all's own quota-keyword short-circuit. Using the bare `APIError` mock exercised that catch-all branch instead (which predates this phase and correctly returns no `resetsAt` — it's for non-429 quota/credits messages, e.g. a 400/403), not the classified-429 path the tests meant to prove. Fixed by constructing real `RateLimitError` instances. Not a shipped bug — caught before the tests were ever considered passing, by reading exactly why they failed rather than loosening the assertions.
- **Live-path plumbing turned out wider than estimated** — the design doc's "three files plus StatusNotice.tsx/PresenceHeader.tsx" undercounted: `DealIntelligenceStatus` is deliberately declared TWICE (`ui/types.ts` and `useDealIntelligence.ts`, on purpose — see that file's own BUG-057-era doc comment on why, and the standing trap it already documented: "keep both in sync by hand, the real typecheck is what catches a missed one"), plus `tier1/analyze.ts`/`tier2/analyze.ts`/`preload/index.d.ts`/`useLiveCues.ts`/`LiveView.tsx`/`DealIntelligencePanel.tsx` all carry their own copy of the same union. All twelve files updated together; the real `npm run typecheck` (both node and web) is clean, which is what actually proves nothing was missed — not the file count matching the original estimate.
- **`anthropic.ts`'s `toProviderError` had no test coverage of its own at all before this** (confirmed by grep — no dedicated anthropic provider test file exists, only cross-provider fallback tests that treat `'anthropic'` as an opaque id) — exported it (same "exported for direct unit testing" convention `openai-compatible.ts` already uses) and added it to `providerErrorDetail.test.ts` alongside the other providers: the later-of-two-headers rule, a single header present, no headers at all, and the non-period-exhausted case getting no resetsAt.
- **Verified**: `messageFor`'s new branch and `markPeriodExhausted`'s `resetsAt` handling both red-checked (disabled, confirmed the specific new tests fail and nothing else does, restored); the `quota-exhausted` pausedReason routing in `live-cue.ts` red-checked the same way. Full suite: 1772 passing, 5 skipped, both typecheck passes clean.
...
- **Tuning `PACING_GAP_MS` per-provider** — ships as one global constant. Revisit with real post-ship data rather than more research now. **DONE — M27 H2 (2026-08-14):** now per-provider (Groq 2s, OpenRouter 3s, everything else the 6s default), derived from each free tier's documented per-request rate; see `model-pacing.ts`'s `PACING_GAP_MS_BY_PROVIDER`.
