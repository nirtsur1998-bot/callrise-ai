> **HISTORICAL SNAPSHOT — the M30 overnight audit handoff, written 2026-08-25.**
> Kept for its findings, not as current state. Renamed from `docs/HANDOFF.md` on
> 2026-09-02 so it cannot be mistaken for *the* handoff: a file with that name on
> `main` reads as current to whoever finds it, and this one is two weeks old.
>
> **Its red banner below is RESOLVED.** BUG-116 shipped in v1.4.0 on 2026-08-28
> (`d569706`, fixed by deletion). No install carries source history. Read the rest
> with that correction in mind, and check anything else here against
> `01-Milestones/Milestone Tracker.md` in the vault before acting on it.
