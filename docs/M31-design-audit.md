...
9b. **Every meeting/task reminder the alerts backend was supposed to fire** — added 2026-08-29 while building M31's reminder fix, on the founder's instruction, because item 9 understates it. Hiding the Alerts *page* hid the controls, not the promise: `meeting_starting` (plus `task_due`, `deal_cold`, `no_next_step`) is dispatched by the same undeployed backend, so **no reminder this app has ever offered has fired for any user since M19**. The desktop path in `alerts.ts` (`subscribeDesktopDeliveries` → `handleDesktopDelivery`) is fully built, even reads a real cached prep brief for its body, and has never executed once — no `alert_deliveries` row is ever created to wake it. This is a *worse* species than the thirteen in group B: those are off and silent; this one is **advertised as on and silently does nothing**. Partially addressed in M31 Slice A by a small local scheduler (`src/main/event-reminders.ts`) that fires while the app is running and says so plainly — deliberately built fresh rather than "reusing" a mechanism that has never worked. See BUG-083's 2026-08-29 addendum.
...
| 4 | Identity pass | Token swap + component sweep + **Windows title-bar overlay** (finally match macOS) + min-size/scaling fixes for 125%/150% + distinctiveness re-test + **fix BUG-133 (contrast) for the new ramp, both themes** + **decide the Speed/Quality lane-label tokens** | M |
...
**Stage 4 acceptance criteria — explicit, added 2026-08-29 per the founder's instruction after BUG-133 was found (not a suggestion, a gate):**
1. Every surface/border/text pair `src/renderer/src/__tests__/contrast-ratios.test.ts` checks clears its WCAG minimum (3:1 non-text UI boundaries, 4.5:1 text) **in both themes**, for whatever hex values First Light lands on — measured by running that file, not eyeballed or asserted from a screenshot. Stage 4 is not done with the ramp while `KNOWN_BUG_133_FAILURES` still has entries in it.
2. The dark-theme question BUG-133 surfaced needs an explicit answer before Stage 4 starts authoring the new dark ramp: is the acceptance bar the same WCAG 2.1 ratio the guard already computes (which would mean today's already-shipping, already-fine-looking dark ramp also needs new values), or a different, perceptually-validated measure (e.g. APCA)? Whichever is chosen, the guard's thresholds get updated to match — not quietly loosened to make today's numbers pass.
3. `ModelAssignmentSection.tsx`'s Speed/Quality lane labels (currently reusing `--color-warning`/`--color-accent`'s *values* as an interim, non-semantic stand-in — see BUG-131) get an explicit decision: their own dedicated tokens, or confirmed as fine to keep borrowing. Not a default-yes; a real founder call, since it's a color decision like any other in this stage.
4. Distinctiveness re-test from §7, using the FIXED (both-theme-legible) screens as the baseline, not today's.
