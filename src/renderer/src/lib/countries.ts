// A static country reference list (ISO 3166-1 alpha-2 code, common name, and
// international calling code). Used by the country-of-client picker and the
// phone-number country-code picker. Flags are rendered from the code (regional
// indicator emoji), so no image assets are needed.
...
/** Converts an ISO2 code into its flag emoji via regional indicator symbols. */
...
export function countryDial(code: string | undefined): string | undefined {
  if (!code) return undefined
