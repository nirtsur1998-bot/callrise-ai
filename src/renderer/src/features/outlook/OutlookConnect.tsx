...
function friendlyError(code: string): string {
  switch (code) {
    case 'no-credentials':
      return 'OUTLOOK_CLIENT_ID is missing from your .env.'
    case 'encryption-unavailable':
      return "This Mac's secure storage isn't available, so the login can't be saved safely."
    case 'access_denied':
      return 'You declined the Microsoft permission. Click Connect to try again.'
    case 'timeout':
      return 'Authorization timed out — click Connect to try again.'
    case 'read-failed':
      return "Couldn't read your calendars — try Refresh."
    case 'write-failed':
      return "Couldn't write to Outlook — try reconnecting two-way sync."
    default:
      return "Couldn't finish connecting to Outlook — click Connect to try again."
  }
}

/**
 * Connect Outlook Calendar (read-only): drives the OAuth IPC, shows status +
 * the calendar list, and calls `onChange` whenever the connection changes
 * (connect / disconnect / refresh) so the calendar can re-pull events. Same
 * shape as GoogleConnect, aimed at Microsoft Graph.
 */
...
export function OutlookConnect({
  ...
  const loadCalendars = async (): Promise<void> => {
    const res = await window.api.outlook.listCalendars()
  ...
  const connect = async (): Promise<void> => {
    ...
    setConnecting(true)
    try {
      const res = await window.api.outlook.connect()
      if (!mounted.current) return
      if (res.ok) {
        await refreshStatus()
  ...
  const enableTwoWaySync = async (): Promise<void> => {
    ...
    try {
      const res = await window.api.outlook.connectWrite()
      if (!mounted.current) return
      if (res.ok) {
        await refreshStatus()
        onChange?.()
  ...
  const disconnect = async (): Promise<void> => {
    await window.api.outlook.disconnect()
    if (!mounted.current) return
    setConnected(false)
    setMode('readonly')
    setCalendars([])
  ...
  if (!configured) {
    return (
      <div className="mb-3 flex items-start gap-2 rounded-xl border border-warning/30 bg-warning-soft px-3.5 py-2.5 text-[13px] text-warning">
        ...
        <span>
          Add <code className="text-warning">OUTLOOK_CLIENT_ID</code> to your <code>.env</code>,
          then restart, to connect Outlook Calendar.
        </span>
      </div>
    )
  }
    ...
    <div className="mb-3 rounded-xl border border-line-soft bg-surface px-4 py-3.5">
      ...
      <div className="flex items-start gap-3">
        <span
          className={cn(
            ...
            connected ? 'bg-track-outlook/15' : 'bg-elevated'
          )}
        >
          <CalendarCheck2
            className={cn('h-4.5 w-4.5', connected ? 'text-track-outlook' : 'text-faint')}
          />
        </span>
        <div className="min-w-0 flex-1 pt-0.5">
          <p className={cn('text-sm font-medium', connected ? 'text-track-outlook' : 'text-ink')}>
            {connected ? 'Connected to Outlook Calendar' : 'Outlook Calendar'}
          </p>
        ...
        {connected ? (
          <>
            {mode === 'readonly' && (
              <Button
                ...
                title="Let CallRise AI add and update events in your Outlook Calendar"
              >
                {enablingSync ? (
                  <>
                    <Loader2 className="h-3.5 w-3.5 animate-spin" /> Waiting for Microsoft…
                  </>
                ) : (
                  <>
          ...
          <Button size="sm" onClick={() => void connect()} disabled={connecting}>
            {connecting ? (
              ...
              <>
                <Plug className="h-3.5 w-3.5" /> Connect Outlook Calendar
              </>
            )}
          </Button>
        )}
      ...
      {(connecting || enablingSync) && (
        <p className="mt-2.5 text-[11px] text-faint">
          A Microsoft sign-in opened in your browser. Approve it there
          {enablingSync && ' and allow the "read and write calendar events" permission'}, then come
      ...
      {connected && calendars.length > 0 && (
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          {calendars.map((c) => (
            ...
            >
              <Check className="h-3 w-3 text-track-outlook" />
              {c.summary}
