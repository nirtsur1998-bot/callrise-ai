...
type ScanResult = { scanned: number; candidatesAdded: number }
 ...
 */
export function ScanPastCallsCard({ enabled }: { enabled: boolean }): React.JSX.Element {
  ...
  const startScan = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        setResult({ scanned: res.scanned, candidatesAdded: res.candidatesAdded })
        await loadEstimate()
      } else {
  ...
  if (eligibleCount === 0) {
    return (
      <p className="text-[13px] text-muted">
        Every past call with a transcript has already been mined. New calls are mined
  ...
  return (
    <div className="flex flex-col items-start gap-3">
      <p className="text-sm text-muted">
        ...
        not been mined yet. Scanning calls Claude once per call — expect {formatMinutes(
          eligibleCount * SECONDS_PER_CALL
        )}{' '}
        and {formatCost(eligibleCount)} in AI cost (a rough estimate, not a guarantee).
      </p>
      ...
      {result ? (
        <p className="text-[13px] text-emerald-400">
          ...
          {result.candidatesAdded} suggestion{result.candidatesAdded === 1 ? '' : 's'} — check the
          review queue above.
        </p>
      ) : (
        <button
        ...
        >
          ...
          {scanning ? 'Scanning…' : `Scan ${eligibleCount} past call${eligibleCount === 1 ? '' : 's'}`}
        </button>
      )}
    </div>
  )
