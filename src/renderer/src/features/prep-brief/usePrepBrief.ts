...
export function usePrepBrief(input: PrepBriefEventInput | null): UsePrepBrief {
  ...
  const [salesBrainEdge, setSalesBrainEdge] = useState<string | null>(null)
  ...
  const load = useCallback(
    async (force: boolean) => {
      ...
      if (result.ok) {
        ...
        setSalesBrainEdge(result.salesBrainEdge ?? null)
      } else {
        setRecord(null)
        setFocusSkillReminder(null)
        setSalesBrainEdge(null)
        setError(
          result.error === 'no-key'
  ...
  return { loading, record, error, focusSkillReminder, salesBrainEdge, regenerate: () => load(true) }
}
