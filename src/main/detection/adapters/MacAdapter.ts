...
function loadNativeAddon(): { addon: NativeAddon | null; error: unknown } {
  try {
    ...
    signalNativeLoad({ module: 'mac-audio-activity', ok: true }) // M29 A2 — once per process
    return { addon, error: null }
  } catch (error) {
    signalNativeLoad({ module: 'mac-audio-activity', ok: false, errorClass: errorClassOf(error) })
