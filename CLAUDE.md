...
- **M12 — Reliable buyer-side audio** (system-audio / loopback capture) is the deliberate next step — needed for true talk-ratio, auto-detecting what the buyer said, and per-client memory across calls. Today everything is **mic-only**: live cues react to the buyer only when their voice reaches the mic. M11 built the consent foundation it depends on. **M12 must confirm consent BEFORE any buyer audio starts** and gate capture on the `recordOtherParty` invariant; note the M11 "Other party" control currently appears only after Start, so the consent step needs to move/gate earlier for capture.
