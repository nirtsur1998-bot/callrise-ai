...
const SCRATCH = join(os.tmpdir(), 'callrise-bug261-profile')
...
app.whenReady().then(async () => {
  ...
  console.log(`[bug261] keys available: ${Object.keys(keys).join(', ')}`)

  const env = { ...process.env, ELECTRON_RUN_AS_NODE: '1' }
  if (process.argv.includes('--ceilings')) env.BUG261_CEILINGS = process.argv[process.argv.indexOf('--ceilings') + 1]
  for (const [k, v] of Object.entries(keys)) env[`BUG261_${k}`] = v

  const child = spawn(process.execPath, [join(__dirname, 'bug261-tight-ceilings-run.cjs')], {
    cwd: REPO,
    env,
